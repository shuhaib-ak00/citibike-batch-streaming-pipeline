"""Producer: poll GBFS station_status -> Kafka.

Setiap polling menghasilkan **snapshot penuh** dan seluruh isinya direkam
(periodic snapshot, bukan CDC). Pesan dipartisi dengan key = station_id
supaya urutan per stasiun konsisten di dalam satu partisi.

Dua hal yang dilakukan di luar publishing:
  1. Payload yang tidak lolos validasi dikirim ke topik DLQ, bukan dibuang.
  2. Snapshot mentah disimpan ke GCS. Ini bukan sekadar arsip: saat demo,
     data ini bisa di-replay bila API GBFS sedang bermasalah.
"""
...
def _delivery_report(err, msg) -> None:
    """Dipanggil Kafka saat pengiriman selesai/gagal."""
    if err is not None:
...
def poll_once(producer: Producer) -> tuple[int, int]:
    ...
    Returns:
        (jumlah_baris_valid, jumlah_baris_ditolak)
    """
    ...
    try:
        ...
        # Kegagalan HTTP bukan kesalahan data, jadi tidak masuk DLQ.
        # Producer akan mencoba lagi pada siklus berikutnya.
    ...
    for item in envelope.data.stations:
        ...
        message = {
            **station.model_dump(),
            "last_reported_iso": (
                station.last_reported_dt.isoformat() if station.last_reported_dt else None
            ),
