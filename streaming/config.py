...
Nilai di-inject oleh `env_file: .env` di docker-compose untuk service
`streaming-producer` dan `streaming-consumer`.
"""
...
# ---------- Perilaku consumer ----------
# Tulis massal jauh lebih efisien daripada satu load job per pesan Kafka.
CONSUMER_BATCH_SIZE = int(env("CONSUMER_BATCH_SIZE", "5000"))
# Batas tunggu bila batch belum penuh, supaya data tidak mengendap lama.
CONSUMER_FLUSH_SECONDS = int(env("CONSUMER_FLUSH_SECONDS", "30"))

# ---------- Path (referensi/replay saat demo) ----------
